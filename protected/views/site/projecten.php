<div class="span8 tabbable">
  <ul class="nav nav-tabs">
    <li>
      <a href="<?php echo Yii::app()->request->baseUrl; ?>/">Biografie</a>
    </li>
    <li class="active">
      <a>Projecten</a>
    </li>
    <li>
      <a href="<?php echo Yii::app()->request->baseUrl; ?>/beelden.html">
          Beelden</a>
    </li>
  </ul>
  <div class="tab-content">
    <div class="tab-pane active" id="projecten">
      <h2>Huidige Projecten</h2>
      <div itemscope itemtype="http://schema.org/Event">
        <h3 itemprop="name">Jamaica Farewell</h3>
        <br>
        <div itemprop="description">
          <blockquote class="">
            <p>
              Down the way where the nights are gay<br>
              And the sun shines daily on the mountain top,
            </p>
            <p>
              I took a trip on a sailing ship<br>
              And when I reached Jamaica I made a stop. 
            </p>
            <p>
              But I'm sad to say, I'm on my way,<br>
              Won't be back for many a day...
            </p>
             <small><cite>Harry Belafonte   in Jamaica Farewell</cite></small>
          </blockquote>
          <p>
            Wat is dat thuisland waar Belafonte over zingt? En hoe komt het dat
            zijn verlangen zo herkenbaar is? Is de zoektocht naar het thuisland
            universeel en van alle tijden? Of is deze verbonden aan de
            globalisatie waar wij vandaag de dag deel van uitmaken? Hoe zoekt
            de ontheemde mens anno 2012 naar de plek waar hij thuis hoort? En
            is deze plek überhaupt bereikbaar, of is het thuisland een utopie?  
          </p>
          <p>
            Jet van Helbergen maakt in samenwerking met schrijfster Marjolijn
            van Heemstra een voorstelling over het grotere verhaal achter dit
            liedje. Een voorstelling over heimwee en verlangen, in alle soorten
            en maten. Met bestaande en nieuw gecomponeerde muziek. Locatie is
            de stille Palmenkas in de Amsterdamse Hortus, vol bomen die
            afkomstig zijn uit andere thuislanden en op hun eigen manier
            verstekelingen.
          </p>
          <dl class="dl-horizontal">
            <dt>Zang en spel:</dt>
            <dd itemprop="performer"
                itemscope
                itemtype="http://schema.org/Person">
              <span itemprop="name">Jet van Helbergen</span>
            </dd>
            <dt>Tekst:</dt>
            <dd itemprop="performer"
                itemscope
                itemtype="http://schema.org/Person">
              <span itemprop="name">Marjolijn van Heemstra</span>
            </dd>
            <dt>Instrumentalisten:</dt>
            <dd><span itemprop="performer"
                itemscope
                itemtype="http://schema.org/Person">
              <span itemprop="name">Boele Weemhoff</span>
            </span> en
            <span itemprop="performer"
                itemscope
                itemtype="http://schema.org/Person">
              <span itemprop="name">Martijn Heida</span>
            </span>
            <dt>Composities:</dt>
            <dd>
              <span itemprop="performer"
                  itemscope
                  itemtype="http://schema.org/Person">
                <span itemprop="name">Roald van Oosten</span>,
              </span>
              <span itemprop="performer"
                  itemscope
                  itemtype="http://schema.org/Person">
                <span itemprop="name">Joost Schouten</span>,
              </span>
              <span itemprop="performer"
                  itemscope
                  itemtype="http://schema.org/Person">
                <span itemprop="name">Boele Weemhoff</span>
              </span> en
              <span itemprop="performer"
                  itemscope
                  itemtype="http://schema.org/Person">
                <span itemprop="name">Martijn Heida</span>
              </span>
            </dd>
            <dt>Eindregie:</dt>
            <dd itemprop="performer"
                itemscope
                itemtype="http://schema.org/Person">
              <span itemprop="name">Bart Kiene</span>
            </dd>
          </dl>
        </div>
        <p>
          <span itemprop="location"
              itemscope
              itemtype="http://schema.org/Zoo">
            <span itemprop="name">Palmenkas, Hortus Botanicus</span>,
            <span itemprop="address"
                itemscope
                itemtype="http://schema.org/PostalAddress">
              <span itemprop="addressLocality">Amsterdam</span>
            </span>
          </span>
          <br>
          <span itemprop="startDate" content="2012-09-17T20:00">
            17 september 2012, Aanvang: 20.00
          </span>
          (try-out)<br>
          19 september 2012, Aanvang: 20.00 (premiere)<br>
          20 september 2012, Aanvang: 20.00<br>
          21 september 2012, Aanvang: 20.00<br>
          23 september 2012, Aanvang: 17.15<br>
          26 september 2012, Aanvang: 20.00 (uitverkocht)<br>
          27 september 2012, Aanvang: 20.00 (uitverkocht)<br>
          28 september 2012, Aanvang: 20.00<br>
          30 september 2012, Aanvang: 17.15<br>
          <br>
          <a href="http://dehortus.nl/programma/#1104">
            Kaartverkoop en informatie</a>
          <br>
          <a href="http://www.voordekunst.nl/vdk/project/view/430-jamaica-farewell">
            Omschrijving op Voor de Kunst</a>
        </p>
      </div>
      <div itemscope itemtype="http://schema.org/Event">
        <h3 itemprop="name">Requiem, W.A. Mozart</h3>
        <p itemprop="description">
          In samenwerking met het
          <span itemprop="performer"
              itemscope
              itemtype="http://schema.org/MusicGroup">
            <span itemprop="name">Noordhollands Muziekensemble</span>
          </span>
          o.l.v.
          <span itemprop="performer"
              itemscope
              itemtype="http://schema.org/Person">
            <span itemprop="name">Hendrik Timmerman</span>.
          </span>
        </p>
        <p>
          <span itemprop="location"
              itemscope
              itemtype="http://schema.org/Church">
            <span itemprop="name">Grote- of St. Laurenskerk</span>,
            <span itemprop="address"
                itemscope
                itemtype="http://schema.org/PostalAddress">
              <span itemprop="addressLocality">Alkmaar</span>
            </span>
          </span>
          <br/>
          <span itemprop="startDate" content="2012-10-20T20:00">
            20 oktober 2012, Aanvang: 20.00
          </span>
          <br/>
          <a href="http://nhme.nl/">Kaartverkoop en informatie</a>
        </p>
      </div>
      <div itemscope itemtype="http://schema.org/Event">
        <h3 itemprop="name">Besloten Kerst-recital</h3>
        <p itemprop="description">
          In samenwerking met pianist
          <span itemprop="performer"
              itemscope
              itemtype="http://schema.org/Person">
            <span itemprop="name">Cyrill Ibrahim</span>.
          </span>
        </p>
        <p>
          <span itemprop="location"
              itemscope
              itemtype="http://schema.org/Place">
            <span itemprop="name">De Beukenhorst</span>,
            <span itemprop="address"
                itemscope
                itemtype="http://schema.org/PostalAddress">
              <span itemprop="addressLocality">
                Driebergen-Rijsenburg
              </span>
            </span>
          </span>
          <br>
          <span itemprop="startDate" content="2012-12-17">
            17 - 24 december 2012
          </span>
          <br>
          <a href="http://cyrillibrahim.wordpress.com/">
            Website van Cyrill Ibrahim</a>
        </p>
      </div>
      <div itemscope itemtype="http://schema.org/Event">
        <h3 itemprop="name">Weihnachtsoratorium, J.S. Bach</h3>
        <p itemprop="description">
          In samenwerking met het 
          <span itemprop="performer"
              itemscope
              itemtype="http://schema.org/MusicGroup">
            <span itemprop="name">Noordhollands Muziekensemble</span>
          </span>
          o.l.v.
          <span itemprop="performer"
              itemscope
              itemtype="http://schema.org/Person">
            <span itemprop="name">Hendrik Timmerman</span>.
          </span>
        </p>
        <p>
          <span itemprop="location"
              itemscope
              itemtype="http://schema.org/Church">
            <span itemprop="name">Ter Coulskerk</span>,
            <span itemprop="address"
                itemscope
                itemtype="http://schema.org/PostalAddress">
              <span itemprop="addressLocality">Heiloo</span>
            </span>
          </span>
          <br>
          <span itemprop="startDate" content="2012-12-22T20:00">
            22 december 2012, Aanvang: 20.00
          </span>
          <br>
          <a href="http://nhme.nl/">Kaartverkoop en informatie</a>
        </p>
      </div>
    </div>
  </div>
</div>

